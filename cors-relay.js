...
/* ======================================================================
   Synchronisation
   ----------------------------------------------------------------------
   Nécessite une base D1 nommée DB. Créer la table une seule fois :
   ...
   Le serveur ne voit que : un identifiant de salon (dérivé du code secret par PBKDF2,
   irréversible), des identifiants d'objets, des horodatages, et du contenu chiffré.
   Il ne peut pas lire les recettes ni les listes.
   ====================================================================== */
...
async function handleSync(request, env) {
  ...
  if (request.method === 'POST') {
    ...
    const stores = ['recipes', 'meals', 'shopping'];
    ...
    for (const it of items) {
      if (!it || typeof it.id !== 'string' || !stores.includes(it.store)) continue;
